...
from models import WideResNet, ModelEMA, FCNet
from utils import (AverageMeter, create_loss_fn, accuracy_moon, save_checkpoint, 
                   reduce_tensor, model_load_state_dict, plot_decision_boundary,
                   plot_data)
...
parser.add_argument('--num_labeled', type=int, default=10, help='number of labeled data')
parser.add_argument("--num_test", type = int, default = 200, help = "number of test samples")
parser.add_argument("--expand-labels", action="store_true", help="expand labels to fit eval steps")
parser.add_argument('--total_steps', default=300000, type=int, help='number of total steps to run')
...
parser.add_argument('--wandb_mode', default='disabled', type=str, help='wandb mode')
...
def train_loop(args, labeled_loader, unlabeled_loader, test_loader, finetune_dataset,
    ...
    logger.info(f"   Task =moon@{args.num_labeled}")
...
def evaluate(args, test_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for step, (images, targets) in enumerate(test_iter):
            ...
            acc = accuracy_moon(outputs, targets)
...
def finetune(args, finetune_dataset, test_loader, model, criterion):
    ...
    for epoch in range(args.finetune_epochs):
        ...
        if args.local_rank in [-1, 0]:
            ...
            args.writer.add_scalar("finetune/test_loss",
                                    test_loss, epoch)
...
def main():
    ...
    if args.local_rank in [-1, 0]:
        ...
        wandb.init(name=args.name, project='MPL', config=args, mode = args.wandb_mode)
    ...
    labeled_dataset, unlabeled_dataset, test_dataset, finetune_dataset = DATASET_GETTERS["moon"](args)
    ...
    input_size, hidden_size = 2, 8

    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()

    teacher_model = FCNet(input_size, hidden_size)
    student_model = FCNet(input_size, hidden_size)
    plot_data(labeled_dataset, unlabeled_dataset, test_dataset)
    plt.show()
    logger.info(f"Model: FCNet {input_size}x{hidden_size}x{hidden_size}x2")
    ...
    plot_decision_boundary(student_model, test_dataset, labeled_dataset)
    plt.show()
